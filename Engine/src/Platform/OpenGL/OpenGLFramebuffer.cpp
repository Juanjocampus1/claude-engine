...
namespace ClaudeEngine {
    ...
    namespace Utils {
        ...
        static GLenum FBTextureFormatToGL(FramebufferTextureFormat format) {
            ...
            CE_CORE_ASSERT(false, "Unknown texture format");
            return 0;
        }
    ...
    void OpenGLFramebuffer::Invalidate() {
        ...
        if (m_ColorAttachments.size() > 1) {
            CE_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Too many color attachments");
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height) {
        if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
            CE_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
            return;
        }
    ...
    int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
        CE_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Attachment index out of bounds");
